"""Evaluation script for trained 24-Point Game GRPO model — GPU version."""
...
def print_eval_results(results: Dict):
    ...
    for i, detail in enumerate(results['details'][:5]):
        status = "SOLVED" if detail.get('is_solved') else ("HALLUC" if detail.get('is_hallucination') else "FAIL")
        if detail.get('error'):
            status = f"ERROR: {detail['error'][:40]}"
        print(f"  [{status}] nums={detail['numbers']} -> expr={detail.get('expression', 'N/A')}")
...
def run_full_evaluation(
    ...
    print("=" * 60)
    print("24-Point Game Model Evaluation (GPU)")
    print("=" * 60)
    ...
    # Evaluate on in-distribution solvable test set
    ...
    if datasets.get('test_ood') and len(datasets['test_ood']) > 0:
        results = evaluate_on_dataset(
...
def main():
    ...
    if args.quick:
        EvalConfig.num_test_samples = 20
